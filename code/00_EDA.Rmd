...
ggplot(grow.df, aes(Holdfast, nextHoldfast)) + geom_abline(colour="grey") +
  geom_point() + stat_smooth(method="lm", formula=y~x+I(x^2)) +
  scale_x_log10() + scale_y_log10()
ggplot(grow.df, aes(Holdfast, nextHoldfast, colour=Management)) + geom_abline(colour="grey") +
  geom_point() + 
  stat_smooth(aes(group=Site), method="lm", formula=y~x+I(x^2), se=F, size=0.25) +
  stat_smooth(method="lm", formula=y~x+I(x^2)) +
  scale_x_log10() + scale_y_log10()

ggplot(grow.df, aes(Stipes, nextStipes)) + geom_abline(colour="grey") +
  geom_point() + stat_smooth(method="lm", formula=y~x+I(x^2)) 
ggplot(grow.df, aes(Stipes, nextStipes, colour=Management)) + geom_abline(colour="grey") +
  geom_point() + 
  stat_smooth(aes(group=Site), method="lm", formula=y~x+I(x^2), se=F, size=0.25) +
  stat_smooth(method="lm", formula=y~x+I(x^2)) 

ggplot(grow.df, aes(Total_Length, nextLength)) + geom_abline(colour="grey") +
  geom_point() + stat_smooth(method="lm", formula=y~x+I(x^2)) +
  scale_x_log10() + scale_y_log10()
ggplot(grow.df, aes(Total_Length, nextLength, colour=Management)) + geom_abline(colour="grey") +
  geom_point() + 
  stat_smooth(aes(group=Site), method="lm", formula=y~x+I(x^2), se=F, size=0.25) +
  stat_smooth(method="lm", formula=y~x+I(x^2)) +
  scale_x_log10() + scale_y_log10()
...
# Recruitment
We can estimate size distributions of recruits by management, and estimate the number of recruits as a function of the number of *other* plants in the plot
```{r recruitment_plots}
rcr.df <- Growth_Surv %>% 
  select(Site, Management, Patch, Monitoring, 
         Plant, Holdfast, Stipes, Total_Length) %>%
...
rcr.df %>% group_by(Site, Management, Patch, Monitoring, nPlants) %>%
  ...
  ggplot(aes(nPlants-nRecruits, nRecruits)) + geom_jitter() +
  stat_smooth(method="lm", formula=y~log1p(x)) +
  scale_y_continuous(trans="log1p") +
  xlab("Number of non-recruits")
rcr.df %>% group_by(Site, Management, Patch, Monitoring, nPlants) %>%
  summarise(nRecruits=sum(newRecruit)) %>%
  ggplot(aes(nPlants-nRecruits, nRecruits, colour=Management)) + geom_jitter() +
...
rcr.df %>% group_by(Site, Management, Patch, Monitoring, totHoldfast) %>%
  ...
  ggplot(aes(totHoldfast-HoldfastRcr, nRecruits)) + geom_jitter() +
  stat_smooth(method="lm", formula=y~log1p(x)) +
  scale_y_continuous(trans="log1p") +
